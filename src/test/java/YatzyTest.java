...
public class YatzyTest
{
    ...
    @Test(dataProvider = "occurences")
    public void testOccurences(final Roll roll, final Integer value, final Long expectedOccurence)
    {
        Yatzy yatzy = new Yatzy(roll);
        Long occurence = yatzy.occurences().get(value);
        assertEquals(occurence, expectedOccurence);
    }

    @DataProvider(name = "occurences")
    public Object[][] addDataProvider()
    {
        return new Object[][]{{new Roll(2, 1, 3, 2, 4), 1, 1L}, {new Roll(2, 1, 3, 2, 4), 2, 2L}};
